 ...
 * @type {string}
 */
const BASE_URL = 'https://github.com/eGiftCard/';

/**
 * Get the name of the workspace. The workspace name is expected to be in the
 * form `@egiftcard/workspace-name`, and this function will extract
 * `workspace-name`.
 *
...
async function expectReadme(workspace, workspaceName) {
  ...
  if (
    workspaceName !== 'egiftcard-module-template' &&
  ...
  if (!readme.includes(`yarn add @egiftcard/${workspaceName}`)) {
    workspace.error(
      `The README.md does not contain an example of how to install the package using Yarn (\`yarn add @egiftcard/${workspaceName}\`). Please add an example.`,
    );
  }

  if (!readme.includes(`npm install @egiftcard/${workspaceName}`)) {
    workspace.error(
      `The README.md does not contain an example of how to install the package using npm (\`npm install @egiftcard/${workspaceName}\`). Please add an example.`,
    );
  }
}
...
async function expectPullRequestTemplate(workspace, workspaceName) {
  if (workspaceName === 'egiftcard-module-template') {
    return;
  }
...
module.exports = defineConfig({
  async constraints({ Yarn }) {
    ...
    expectWorkspaceField(workspace, 'name', `@egiftcard/${workspaceName}`);
    ...
    // a repository within the eGiftCard organization.
